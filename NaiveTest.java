...
public class NaiveTest 
{
    public static boolean isPrime(BigInteger candidate) 
    {
        ...
        */
        ...
        // 2 is a prime
        if (candidate == BigInteger.valueOf(2))
        {
          return true;
        }
        //returns false if not a prime
        for (BigInteger i = BigInteger.valueOf(2); i.pow(2).compareTo(candidate) <= 0; i = i.add(BigInteger.ONE))
        {
          if (candidate.mod(i).equals(BigInteger.ZERO) == true)
          {
            return false;
          }
        }
